...
public class BuyRocket : MonoBehaviour
{
    public string Tag = "";
    ...
    void OnGUI()
    {
        ...
        {
            GUI.Box(new Rect((Screen.width) / 2 - (Screen.width) / 8, (Screen.height) / 2 - (Screen.height) / 8, (Screen.width) / 4, (Screen.height) / 4), "Press [F] to buy Rocket launcher ammo for " + Ammocost, style);
        }
        if (triggered == true && Purchased == false)
        {
            GUI.Box(new Rect((Screen.width) / 2 - (Screen.width) / 8, (Screen.height) / 2 - (Screen.height) / 8, (Screen.width) / 4, (Screen.height) / 4), "Press [F] to buy Rocket launcher for " + cost, style);
        }
    }
    private void Update()
    {
        ...
        if (GameUI.Rocket.RemainingAmmo < 8)
        {
            needAmmo = true;
        }
        ...
        {
            GameUI.Rocket.Unlocked = true;
            GameUI.WeaponIndex = 4;
