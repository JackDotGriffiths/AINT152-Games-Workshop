...
public class BuyRifle : MonoBehaviour
{
    ...
    void OnGUI()
    {
        ...
        {
            GUI.Box(new Rect((Screen.width) / 2 - (Screen.width) / 8, (Screen.height) / 2 - (Screen.height) / 8, (Screen.width) / 4, (Screen.height) / 4), "Press [F] to buy Rifle ammo for " + Ammocost, style);
        }
        if (triggered == true && Purchased == false)
        {
            GUI.Box(new Rect((Screen.width) / 2 - (Screen.width) / 8, (Screen.height) / 2 - (Screen.height) / 8, (Screen.width) / 4, (Screen.height) / 4), "Press [F] to buy Rifle for " + cost, style);
        }
    }
    private void Update()
    {
        ...
        if (GameUI.Rifle.RemainingAmmo < 200)
        {
            needAmmo = true;
        }
        ...
        {
            GameUI.Rifle.Unlocked = true;
            GameUI.WeaponIndex = 5;
